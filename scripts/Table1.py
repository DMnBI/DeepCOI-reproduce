...
for phylum in phyla:
    ...
    models =  [f"{base}.{phylum}" for base in ['onehot', 'random', 'end_to_end', 'DeepCOI']]
...
df['embedding'] = df['pivot'].apply(lambda x: x.split('.')[0])
df['phylum'] = df['pivot'].apply(lambda x: x.split('.')[1])
df['rank'] = df['pivot'].apply(lambda x: x.split('.')[2])
df = df[['embedding', 'phylum', 'rank', 'AUROC', 'AUPR']]

df.to_csv("Table1.data.csv", index=False)
