...
for phylum in phyla:
    ...
    models =  [
        f"{model}.{phylum}" for model in 
        [
        'no_network.convolutional_attention', 
        '1D-CNN.max', 
        'GRU.convolutional_attention', 
        '1D-CNN_GRU.convolutional_attention',
        ]
    ]
...
pool = mp.Pool(4)
for curves, model in tqdm(pool.imap_unordered(call_func, cmds), total=len(cmds), desc='Table2'):
    auroc = curves['AUROC']
    aupr = curves['AUPR']
...
df.to_csv("Table2.data.csv", index=False)
