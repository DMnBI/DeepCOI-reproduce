...
for phylum in phyla:
    ...
    models =  [f"{phylum}-cnn{w}" for w in [5, 7, 9, 11, 13]]
    
    for model in models:
        cmds.append((meta_file, model))

pool = mp.Pool(5)
for curves, model in tqdm(pool.imap_unordered(call_func, cmds), total=len(cmds), desc='Sup_Table_S4'):
    auroc = curves['AUROC']
    aupr = curves['AUPR']
...
df['phylum'] = df['pivot'].apply(lambda x: x.split('.')[0].split('-')[0])
df['width'] = df['pivot'].apply(lambda x: x.split('.')[0].split('-')[1])
df['rank'] = df['pivot'].apply(lambda x: x.split('.')[1])
df = df[['phylum', 'width', 'rank', 'AUROC', 'AUPR']]

df.to_csv("Sup_Table_S8.data.csv", index=False)
