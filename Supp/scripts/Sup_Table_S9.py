...
for phylum in phyla:
    ...
    models =  [f"DeepCOI-{phylum}-{i}" for i in range(5)]
    
    for model in models:
        cmds.append((meta_file, model))

pool = mp.Pool(5)
for curves, model in tqdm(pool.imap_unordered(call_func, cmds), total=len(cmds), desc='Sup_Table_S9'):
    auroc = curves['AUROC']
    aupr = curves['AUPR']
...
df['trial'] = df['pivot'].apply(lambda x: x.split('.')[0].split('-')[2])
df['rank'] = df['pivot'].apply(lambda x: x.split('.')[1])
df = df[['phylum', 'trial', 'rank', 'AUROC', 'AUPR']]

df.to_csv("Sup_Table_S9.data.csv", index=False)
