...
for phylum in phyla:
    ...
    models =  [f"w_mcm.{m}.{phylum}" for m in ['w_mcm', 'wo_mcm']]
    models += [f"wo_mcm.{m}.{phylum}" for m in ['w_mcm', 'wo_mcm']]
    
    for model in models:
        cmds.append((meta_file, model))

pool = mp.Pool(4)
for curves, model in tqdm(pool.imap_unordered(call_func, cmds), total=len(cmds), desc='Sup_Table_S3'):
    auroc = curves['AUROC']
    aupr = curves['AUPR']
...
df['train'] = df['pivot'].apply(lambda x: x.split('.')[0])
df['test'] = df['pivot'].apply(lambda x: x.split('.')[1])
df['phylum'] = df['pivot'].apply(lambda x: x.split('.')[-2])
df['rank'] = df['pivot'].apply(lambda x: x.split('.')[-1])
df = df[['train', 'test', 'phylum', 'rank', 'AUROC', 'AUPR']]

df.to_csv("Sup_Table_S3.data.csv", index=False)
